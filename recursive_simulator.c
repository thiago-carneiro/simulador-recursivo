                                       ...
                                       float min_rho, float max_rho)
{
  ...
  do
  {
    float depth = ((rand() / RAND_MAX) * (max_depth - min_depth)) + min_depth;
    float amplitude = (rand() / RAND_MAX) * max_depth;
    float k = M_PI / ((rand() / RAND_MAX) * 50);
    float phase = 2 * M_PI * (rand() / RAND_MAX);
    float layer_speed = ((rand() / RAND_MAX) * (max_speed - min_speed)) + min_speed;
    float layer_rho = ((rand() / RAND_MAX) * (max_rho - min_rho)) + min_rho;
    current_z -= depth;
    for (unsigned int x = 0; x < nx; x++)
    {
      int layer_bottom =
  ...
  for (unsigned int x = 0; x < nx; x++)
  {
    ...
    while (z < nz)
    {
      float current_speed = speed[x][z];
      float current_rho = rho[x][z];
      unsigned int current_depth = 0;
      while ((z + current_depth < nz) &&
             (current_speed == speed[x][z + current_depth]) &&
             (current_rho == rho[x][z + current_depth]))
      {
        current_depth++;
      }
      n_layers++;
...
int main()
{
  geological_model_2d model = new_senoidal_model(512, 256, 500., 1000., 1500., 4500., 1., 2.5);
